...
![LSD_step1](/img/algorithms/radix_sort/LSD_step1.jpg)
...
- 키의 수를 $n$, 버켓의 종류 수를 $k$라고 했을 때, 공간복잡도는 $O(n+k)$ 이다.
...
> A. 키의 개수가 $n$, 키의 최대 길이가 $d$, 버킷의 종류가 $k$라고 할 때, 시간복잡도는 $O(n*d)$ 공간복잡도는 $O(n+k)$ 이다.
